...
m1=lm(facs$CD4..T.cells~facs$CD8..T.cells)
abline(m1)
...
m1=lm(macrophages~CD8..T.cells, data=subset(facs, tissues=="DCIS"))
abline(m1, col="#FF6FCF")
...
facs$CD48tot=facs$CD4..T.cells+facs$CD8..T.cells
m1=lm(dendritic.cells~CD48tot, data=subset(facs, tissues=="TN"))
abline(m1, col="red")
...
m1=lm(NK.T.cells~gammadelta.Tcells, data=subset(facs, tissues=="HER2"))
abline(m1, col="#8000FF")
m1=lm(NK.T.cells~gammadelta.Tcells, data=subset(facs, tissues=="NP"))
abline(m1, col="#008040")
...
Estimating the expected proportion of cells: 

Expected number of positive Tregs using uniform spatial distribution:

```{r, cache=T}
cd45no=seq(100, to=30000, by=100)
cd3dist=rnorm(200, 0.55, 0.04)
tregdist=rpois(500, 4)/100
testout=sapply(1:300, function(x) sapply(cd45no, function(x) x*sample(cd3dist,1)*sample(tregdist,1)))
testQ=sapply(1:nrow(testout), function(x) quantile(testout[x, ], c(0.05, 0.95)))
dataOut=data.frame(ncells=cd45no, expect=rowMedians(testout), ylow=testQ[1, ], yupp=testQ[2, ])
ggplot(dataOut, aes(x=ncells, y=expect))+geom_smooth(method=lm, se=T)+
  geom_ribbon(aes(ymin=ylow, ymax=yupp,alpha=0.1))+geom_vline(aes(xintercept=1000))+
  ylab("expected no Tregs")+xlab("No CD45+ cells")+ggtitle("expected distribution of Tregs using FACS")
ggplot(dataOut, aes(x=ncells, y=expect))+geom_smooth(method=lm, se=F)+
  geom_ribbon(aes(ymin=ylow, ymax=yupp,alpha=0.1))+coord_cartesian(xlim = c(0, 750), ylim=c(0, 50))+ 
  ylab("expected no Tregs")+xlab("No CD45+ cells")+ggtitle("expected distribution of Tregs in IF slide")
...
true.ps.1 <- dist.ps.1 <- matrix(NA, nrow = 500, ncol = length(ps))
...
for (b in 1:500) {
    ...
    for (p in ps) {
        ...
        est.p <- NULL
        for (s in ss) {
            indices <- which(z >= s - 4e-4 & z <= s + 4e-4)
            p.hat <- mean(type[indices])
            est.p <- c(est.p, p.hat)
        }
        dist.p <- mean((est.p / true.p - 1)^2)
        dist.ps.1[b, which(ps == p)] <- dist.p
        true.ps.1[b, which(ps == p)] <- true.p
    }
}

### focal to global
sizes <- seq(0.18, 0.5, 0.01)
true.ps.2 <- dist.ps.2 <- matrix(NA, nrow = 500, ncol = length(sizes))
...
for (b in 1:500) {
    ...
    p <- 0.05
    for (size in sizes) {
        range <- c(0.5 - size, 0.5 + size)
        indices.range <- which(x >= range[1] & x <= range[2] & y >= range[1] & y <= range[2] & z >= range[1] & z <= range[2])
        p.new <- min(p * (n / length(indices.range)), 1)
        type.ind <- rbinom(length(indices.range), 1, p.new)
        type <- rep(0, n)
        type[indices.range[which(type.ind == 1)]] <- 1
        r <- 5
        ss <- runif(r)
        true.p <- mean(type)
        est.p <- NULL
        ...
        dist.p <- mean((est.p / true.p - 1)^2)
        dist.ps.2[b, which(sizes == size)] <- dist.p
        true.ps.2[b, which(sizes == size)] <- true.p
    }
}

plot(colMeans(true.ps.1), colMeans(dist.ps.1), type = 'l', lwd = 3, xlab = 'Population proportion p', ylab = 'Squared errors', col = 'dodgerblue2', axes = FALSE)
axis(1)
axis(2, las = 2)
box()
for (b in 1:500) {
    lines(true.ps.1[b, ], dist.ps.1[b, ], col = 'lightgrey')
}
lines(colMeans(true.ps.1), colMeans(dist.ps.1), lwd = 3, col = 'dodgerblue2')

plot(sizes, colMeans(dist.ps.2), type = 'l', lwd = 3, xlab = 'Evenness', ylab = 'Squared errors', col = 'dodgerblue2', axes = FALSE)
axis(1)
axis(2, las = 2)
box()
for (b in 1:500) {
    lines(sizes, dist.ps.2[b, ], col = 'lightgrey')
}
lines(sizes, colMeans(dist.ps.2), lwd = 3, col = 'dodgerblue2')
```
...
```{r}
aovres=aov(value~type, data=subset(CresT2std,  Var1=="T.cells.CD8"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std, Var1=="T.cells.gamma.delta"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std,  Var1=="T.cells.regulatory..Tregs."))
summary(aovres)
TukeyHSD(aovres)
aovres=aov(value~type, data=subset(CresT2std, Var1=="T.cells.CD4.naive"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std,  Var1=="T.cells.CD4.memory.activated"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std,  Var1=="T.cells.CD4.memory.resting"))
summary(aovres)
```
...
```{r}
aovres=aov(value~type, data=subset(CresT2std, type!="IDC" & Var1=="T.cells.CD8"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std, type!="IDC" & Var1=="T.cells.gamma.delta"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std, type!="IDC" & Var1=="T.cells.regulatory..Tregs."))
summary(aovres)
TukeyHSD(aovres)
aovres=aov(value~type, data=subset(CresT2std, type!="N" & Var1=="T.cells.CD4.naive"))
summary(aovres)
aovres=aov(value~type, data=subset(CresT2std, type!="IDC" & Var1=="T.cells.CD4.memory.activated"))
summary(aovres)
TukeyHSD(aovres)
aovres=aov(value~type, data=subset(CresT2std, type!="IDC" & Var1=="T.cells.CD4.memory.resting"))
summary(aovres)
aovres=aov(value~Var1+type, data=subset(CresT2std, type!="IDC"))
summary(aovres)
```

Run Tukey test to determine which is different
...
summary(factor(distance.tigit[ ,2]))
```
