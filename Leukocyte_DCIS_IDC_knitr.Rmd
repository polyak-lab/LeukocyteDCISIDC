...
* FACS and I-FISH data (generated in house):
    + FACS was performed on XX patient samples, separating cells according to CD markers
    + IF was conducted on a cohort of 
    + FISH for CCL cluster and ERBB2 was performed on a cohort of DCIS (XX samples, YY areas) and matched DCIS-IDC (XX samples, YY areas)

* RNA-seq data: 
    + RNA-seq was performed on T-cells isolated from a set of XX patients in house
...
Look at the heatmap output:

```{r, echo=F, eval=F}
# List of genes differentially expressed in both edger and deseq2
GeneListInt=unique(c(rownames(tabCS), rownames(tabC), rownames(tabA),
                     rownames(tabB)))
...
heatmap.2(logCPM[GeneListInt[x1], ], scale = "none", col=HMPallete[11:1], ColSideColors = ColSide, trace="none")
```
...
% Also look for genes which are associated with activation, exhaustion signatures:

```{r, echo=F, eval=F}
HitList=(c(GeneListAct$Activated, GeneListAct$Inhibitory, GeneListAct$Cytotoxic,
          GeneListAct$Exhausted, GeneListAct$Naive))
RowSide=c(rep("red", length(GeneListAct$Activated)), rep("blue", length(GeneListAct$Inhibitory)), rep("pink", length(GeneListAct$Cytotoxic)),
          rep("darkblue", length(GeneListAct$Exhausted)), rep("grey",length(GeneListAct$Naive)))
HitList2=unique(c(GeneListAct$Activation, GeneListAct$Dysfunction))
RowSide2=c(rep("red", length(GeneListAct$Activation)), rep("blue", length(GeneListAct$Dysfunction)))

heatmap.2(logCPM[HitList, ], scale = "none", col=HMPallete[11:1], ColSideColors = ColSide, trace="none", RowSideColors = RowSide)

heatmap.2(logCPM[HitList2, ], scale = "none", col=HMPallete[11:1], ColSideColors = ColSide, trace="none", RowSideColors = RowSide2)
```

% Also narrow down the list only genes which are significant:

```{r, echo=F, eval=F}
a1=as.character(GeneListInt[as.character(GeneListInt) %in% c(HitList, HitList2)])
RS2=rep("red", length(a1))
RS2[a1%in%GeneListAct$Dysfunction]="blue"
RS2[a1%in%GeneListAct$Inhibitory]="blue"
RS2[a1%in%GeneListAct$Exhausted]="darkblue"
RS2[a1%in%GeneListAct$Naive]="grey"
heatmap.2(logCPM[match(a1, rownames(logCPM)), ], scale = "none", col=HMPallete[11:1], ColSideColors = ColSide, trace="none",
          RowSideColors = RS2)
```
...
```{r, eval=F, echo=F}
#Boxplot of the different cell types across the different sets:
ggplot(CresT2, aes(x=Var1 , y=value, fill=Var1))+geom_boxplot(stat="boxplot")+theme(axis.text.x = element_text(angle = 90))+facet_grid(~type,drop=TRUE, scale="free", space="free_x")+theme_bw()+scale_fill_brewer(palette = "Accent")
```



```{r, eval=F, echo=F}
#Separate the different subtypes:
CresT2$type[which(CresT2$Var2%in%l2)]="TN"
CresT2$type[which(CresT2$Var2%in%l1)]="HER2"
ggplot(CresT2, aes(x=Var1 , y=value, fill=Var1))+geom_boxplot(stat="boxplot")+theme(axis.text.x = element_text(angle = 90))+facet_grid(~type,drop=TRUE, scale="free", space="free_x")+theme_bw()+scale_fill_brewer(palette = "Accent")
```
...
```{r, eval=F}
#CCVals2=ddsCounts[na.omit(match(c(GeneListAct$Activation, GeneListAct$Dysfunction), rownames(logCPM))),  ]
#a1=heatmap.2(log(CCVals2+1), col=HMPallete[11:1], ColSideColors = Ki67Vals, RowSideColors = RSC, scale="none", trace="none")

CCVals2=newvstCounts[na.omit(match(c(GeneListAct$Activation, GeneListAct$Dysfunction), rownames(logCPM))),  ]
a1=heatmap.2(CCVals2, col=HMPallete[11:1], ColSideColors = Ki67Vals, RowSideColors = RSC, trace="none")
```
...
```{r FigS6B, echo=F}
par(mfrow = c(1, 1), las = 1, tcl = .1, xaxs = 'i', yaxs = 'i')
...
text(1.5, 60, 'p-value = 0.047')
lines(c(3, 4), c(54, 54), lwd = 2)
text(3.5, 60, 'p-value = 0.003')
...
```{r, eval=F}
# Narrow this down to just samples which are basal-like
l1=which(MatchData$Subtype=="Basal")

mod2A=glm(ActScore~CN+log(Neo+1)+CNA+EST+log(Age), data=MatchData[l1, ])
A1=summary(mod2A)
mod2C=glm(CytoScore~CN+log(Neo+1)+CNA+EST+log(Age), data=MatchData[l1, ])
A2=summary(mod2C)
mod2E=glm(ExhScore~CN+log(Neo+1)+CNA+EST+log(Age), data=MatchData[l1, ])
A3=summary(mod2E)
mod2I=glm(InhScore~CN+log(Neo+1)+CNA+EST+log(Age), data=MatchData[l1, ])
A4=summary(mod2I)
```

Use the leaps package to find the best model for CCL amplification

```{r, echo=F, eval=F}
regsubsets.out <-
    regsubsets(CN~log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData,
...
Do the same for actviation status etc

```{r, echo=F, eval=F}
regsubsets.act <-
    regsubsets(ActScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
summary(regsubsets.act)
par(mfrow=c(1,2))
plot(regsubsets.act, scale="adjr2", main="adj R^2")
plot(regsubsets.act, scale="bic", main="BIC")

regsubsets.cyt <-
    regsubsets(CytoScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
summary(regsubsets.cyt)
par(mfrow=c(1,2))
plot(regsubsets.cyt, scale="adjr2", main="adj R^2")
plot(regsubsets.cyt, scale="bic", main="BIC")

regsubsets.exh <-
    regsubsets(ExhScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
summary(regsubsets.exh)
par(mfrow=c(1,2))
plot(regsubsets.exh, scale="adjr2", main="adj R^2")
plot(regsubsets.exh, scale="bic", main="BIC")

regsubsets.inh <-
    regsubsets(InhScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
summary(regsubsets.inh)
par(mfrow=c(1,2))
plot(regsubsets.inh, scale="adjr2", main="adj R^2")
plot(regsubsets.inh, scale="bic", main="BIC")
```
...
```{r, eval=F}
ActMarkers=TCGAlogCPM[c("GZMB", "IFNG", "GZMA", "PTPRC"), c(na.omit(m1), na.omit(m2), m3)]
ActMarkers=data.frame(t(ActMarkers), Grp=c(rep("her2-like", length(na.omit(m1)) ), rep("lum-like", length(na.omit(m2))), rep("lumHER2amp", length(m3))), Name=colnames(ActMarkers))

act2=melt(ActMarkers)

ggplot(aes(y = value, x = Grp, fill = Grp), data = act2) +geom_boxplot()+ facet_wrap(~variable)+ theme_bw()
...
```{r, eval=F}
## assess entire cohort

mod2A=glm(ActScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData)
A1=summary(mod2A)
mod2C=glm(CytoScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData)
A2=summary(mod2C)
mod2E=glm(ExhScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData)
A3=summary(mod2E)
mod2I=glm(InhScore~CN+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData)
A4=summary(mod2I)
```
...
regsubsets.out <-
    regsubsets(CN~ERBB2+log(Neo+1)+CNA+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
...
plot(regsubsets.cyt, scale="adjr2", main="adj R^2")
plot(regsubsets.cyt, scale="bic", main="BIC")
mod1B=glm(CytoScore~ERBB2+Subtype+EST, data=MatchData[MatchData$Subtype!="Normal", ])
X=summary(mod1B)
A=coefficients(X)

regsubsets.exh <-
    regsubsets(ExhScore~CN+log(Neo+1)+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
...
regsubsets.inh <-
    regsubsets(InhScore~CN+log(Neo+1)+Subtype+EST+log(Age), data=MatchData,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")
...
```{r, echo=F, eval=F}
#Check for scores only in the HER2 cases:
mod1B=glm(InhScore~CN+EST+log(Neo+1)+log(Age), data=MatchData[MatchData$Subtype=="Her2", ])
X=summary(mod1B)
A=coefficients(X)
```
...
%Combined heatmap using both PD1 info and CCL

```{r, eval=F}
Ndata=cbind(OsloGistLogR[ ,na.omit(a1)], OsloGist2[ ,c(4:5)])
Ndata[which(Ndata>2, arr.ind=T)]=1.5

a1=heatmap.2(data.matrix(Ndata), col=HMPallete[11:1], Colv = NA,scale="none",trace="none", RowSideColors =as.character(as.numeric(OsloGist2$tissue)))

a2=c(intersect(a1$rowInd, nidx), intersect(a1$rowInd, which(OsloGist2$tissue=="IDC")))

l1=c(a1$rowInd[OsloGist2$tissue=="DCIS"], a1$rowInd[OsloGist2$tissue=="IDC"])

heatmap.2(data.matrix(Ndata[a2, ]), col=HMPallete[11:1], Colv = NA,Rowv=NA, scale="none",trace="none", RowSideColors =as.character(as.numeric(OsloGist2$tissue[a2])))

heatmap.2(data.matrix(Ndata[a2, ]), col=HMPallete[11:1], scale="none",Colv = NA,Rowv=NA, trace="none", RowSideColors =OsIdNew[a2])           
```
...
beeswarm(prop.mat.slides$Her2.DCIS ~ prop.mat.slides$Her2.Sample_ID, 
         col = 'pink1', xlab = 'Patient IDs', ylab = 'Proportion of cells with chemokine cluster amplification', lwd = 1.5, cex.lab = 1.5, pch = 19, labels = unique(prop.mat.slides$Sample_ID), main = 'DCIS-Only Cohort', cex = 1.5, at = unique(prop.mat.slides$Her2.Sample_ID), xlim = c(-5, 395), cex.main = 1.5)
...
#folders=unique(leukocytes$Pathology.No)
...
leukocytes.ave.cd3[is.na(leukocytes.ave.cd3)]=0
